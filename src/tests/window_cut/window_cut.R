...
LInSARRFE<-function(imageRaster, coherence_map, param, eth, xi=0.9){
  ...
  filter_3x3_mod <- function(image, psi_epsilon) {
  ...
  unwrap_phase <- function(phase_matrix) {
    unwrapped <- matrix(0, nrow = nrow(phase_matrix), ncol = ncol(phase_matrix))
    for (i in 1:nrow(phase_matrix)) {
      for (j in 1:ncol(phase_matrix)) {
        if (j > 1) {
          diff_x <- phase_matrix[i, j] - phase_matrix[i, j - 1]
          unwrapped[i, j] <- unwrapped[i, j - 1] + ifelse(
            diff_x > pi, diff_x - 2 * pi,
            ifelse(diff_x < -pi, diff_x + 2 * pi, diff_x)
          )
        } else if (i > 1) {
          diff_y <- phase_matrix[i, j] - phase_matrix[i - 1, j]
          unwrapped[i, j] <- unwrapped[i - 1, j] + ifelse(
            diff_y > pi, diff_y - 2 * pi,
            ifelse(diff_y < -pi, diff_y + 2 * pi, diff_y)
          )
        } else {
          unwrapped[i, j] <- phase_matrix[i, j]
        }
      }
    }
    return(unwrapped)
  ...
  for (i in margin:(nrow(imageRaster) - marginm1)) {
    for (j in margin:(ncol(imageRaster) - marginm1)) {
      ...
      window_selected_filtered_3x3<-filter_3x3_mod(window_selected, psi_epsilon)
      
      unwrap_phase <- unwrap_phase(window_selected_filtered_3x3)
      ...
      b <- 1- (sigma_v_squared/var.circular(as.vector(unwrap_phase)))
      b <- pmax(0, pmin(1, b)) # Limit b between 0 and 1
      
      psi_estimated<-mean.circular(unwrap_phase) + b*(window_selected_filtered_3x3-mean.circular(unwrap_phase))
      window_selected_filtered_3x3b<-unwrap_phase
