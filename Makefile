...
compile:
	 $(CXX) -march=native -fconstexpr-depth=$(CONST_EXPR_DEPTH) -std=c++$(CXX_VERSION) src/main.cpp -o exec.elf $(CXX_FLAGS)

release: 
	$(CXX) -march=native -fconstexpr-depth=$(CONST_EXPR_DEPTH) -O3 -std=c++$(CXX_VERSION) -DNDEBUG src/main.cpp -o prod_binary $(CXX_FLAGS)
run: compile
	./exec.elf

testO0:
	$(CXX) -std=c++$(CXX_VERSION) -fconstexpr-depth=$(CONST_EXPR_DEPTH) -O0 -march=native src/tests/test.cpp -o test.elf -I"$(R_TEST)" -I"$(RCPP_TEST)" -L"$(R_LIBS_TEST)" $(RINSIDE_CXX_TEST) $(RINSIDE_LDF_TEST) -lR $(CXX_FLAGS_TEST) && ./test.elf --abort #--rng-seed 3144853315

testO3:
	$(CXX) -std=c++$(CXX_VERSION) -fconstexpr-depth=$(CONST_EXPR_DEPTH) -O3 -march=native src/tests/test.cpp -o test_optimized.elf -I"$(R_TEST)" -I"$(RCPP_TEST)" -L"$(R_LIBS_TEST)" $(RINSIDE_CXX_TEST) $(RINSIDE_LDF_TEST) -lR $(CXX_FLAGS_TEST) && ./test_optimized.elf --abort
...
benchmark:
	$(CXX) -std=c++$(CXX_VERSION) -fconstexpr-depth=$(CONST_EXPR_DEPTH) -O3 -march=native -DNDEBUG src/benchmark/benchmark.cpp -o benchmark.elf $(CXX_FLAGS_BENCHMARK) && ./benchmark.elf --benchmark_repetitions=2 --benchmark_report_aggregates_only=true --benchmark_out=benchmark.json --benchmark_out_format=json

benchmark_paper:
	$(CXX) -std=c++$(CXX_VERSION) -fconstexpr-depth=$(CONST_EXPR_DEPTH) -O3 -march=native -DNDEBUG src/benchmark/benchmark.cpp -o benchmark.elf $(CXX_FLAGS_BENCHMARK) && ./benchmark.elf --benchmark_min_warmup_time=2 --benchmark_repetitions=40 --benchmark_report_aggregates_only=true --benchmark_out=benchmark.json --benchmark_out_format=json
